...
class HealthDataWorker(context: Context, workerParams: WorkerParameters) : CoroutineWorker(context, workerParams) {
    ...
    private suspend fun aggregateHealthData(userId: UUID) = coroutineScope {
        ...
        val exerciseSessionFetcher = ExerciseSessionFetcher(applicationContext, userId)
        val heartRateFetcher = HeartRateFetcher(applicationContext, userId)
        val activeCaloriesBurnedFetcher = ActiveCaloriesBurnedFetcher(applicationContext, userId)
        ...
        stepsFetcher.fetchData()
        distanceFetcher.fetchData()
        floorsClimbedFetcher.fetchData()
        exerciseSessionFetcher.fetchData()
        heartRateFetcher.fetchData()
        activeCaloriesBurnedFetcher.fetchData()
        totalCaloriesBurnedFetcher.fetchData()
        speedFetcher.fetchData()
        vo2MaxFetcher.fetchData()
        bodyFatFetcher.fetchData()
        leanBodyMassFetcher.fetchData()
        weightFetcher.fetchData()
        heightFetcher.fetchData()
        boneMassFetcher.fetchData()
        bloodPressureFetcher.fetchData()
        bloodGlucoseFetcher.fetchData()
        bodyTemperatureFetcher.fetchData()
        oxygenSaturationFetcher.fetchData()
        respiratoryRateFetcher.fetchData()
        hydrationFetcher.fetchData()
        nutritionFetcher.fetchData()
        basalMetabolicRateFetcher.fetchData()
        powerFetcher.fetchData()
        sleepSessionFetcher.fetchData()
        menstruationFetcher.fetchData()
        intermenstrualBleedingFetcher.fetchData()
    }
